...
def preprocess(dataset_path):
    ...
    for row in tqdm(df.itertuples()):
        tfrecord_path = os.path.join(dataset_path+"/frame", f"{row.yt8m_file}.tfrecord")
        print(row)
        if not os.path.exists(tfrecord_path):
            # print(f"Warning: {tfrecord_path} does not exist. Skipping...")
            continue
        print(f"Success: {tfrecord_path} exist.")
