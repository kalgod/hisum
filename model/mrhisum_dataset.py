...
class MrHiSumDataset(Dataset):
    ...
    def __init__(self, mode):
        ...
        self.filtered_keys = self.filter_data()
        print(f'Number of {mode} videos: {len(self.filtered_keys)}, {mode} keys: {self.filtered_keys}')

    def filter_data(self):
        """ Filters out data items with missing features or gtscores. """
        filtered_keys = []
        for video_name in self.data[self.mode + '_keys']:
            if f'{video_name}/features' in self.video_data:
                filtered_keys.append(video_name)
        return filtered_keys

    def __len__(self):
        """ Function to be called for the `len` operator of `VideoData` Dataset. """
        # self.len = len(self.data[self.mode+'_keys'])
        self.len = len(self.filtered_keys)
        return self.len
    ...
    def __getitem__(self, index):
        ...
        video_name = self.filtered_keys[index]
        d = {}
        ...
        if self.mode != 'train':
            ...
            d['change_points'] = cps
            d['n_frame_per_seg'] = np.array([cp[1] - cp[0] for cp in cps[:-1]] + [cps[-1][1] - cps[-1][0] + 1])
